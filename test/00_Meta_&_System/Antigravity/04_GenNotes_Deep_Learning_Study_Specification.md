# 👑 Workspace Rules & Master Blueprint

trigger: always_on

## 🎯 Tag & Command Router

| Tag / Prefix | Normalized Mode | Mandatory Behavior & Scope |
| :--- | :--- | :--- |
| **`#gennotes`** | **`GENNOTES`** | **Master Deep Learning & Engineering Study Notes Generator**. Activated on `#gennotes`, `!gennotes`, `/gennotes`, `[GENNOTES]`, `gennotes:`, or whenever asked to transform slides, PDFs, lectures, transcripts, or documentation into learning artifacts. Strictly executes the 55-section study specification below. |
| **`#genc`** | **`GENC`** | Active VS Code editor file query / analysis mode. Inspects active file and provides atomic, first-principles response. |
| **`#build`** | **`BUILD`** | Production-grade implementation mode with strict build contracts, vulnerability breakdowns, and zero dummy code. |
| **`#debug`** | **`DEBUG`** | AST/runtime root-cause diagnosis, defensive fix, and edge case audit. |
